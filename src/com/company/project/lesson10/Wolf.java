package com.company.project.lesson10;

import java.util.Arrays;
import java.util.Objects;

public class Wolf {public class Wolf extends WildAnimal{
    private final String[] likeToEat;
    private final String color;

    public Wolf(String[] likeToEat, String color) {
        this.likeToEat = likeToEat;
        this.color = color;
    }
    @Override
    public boolean equals(Object wolf) {
        if (this == wolf) return true;
        if (!(wolf instanceof Wolf wolf)) return false;
        if (!Arrays.equals(likeToEat, wolf.likeToEat)) return false;
        return Objects.equals(color, wolf.color);
    }
}
